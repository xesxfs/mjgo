...
class LoginController extends BaseController {
	...
	private showLoginScene() {
		...
		this.bLogin = false;
			...
			//1正式服,2测试服
			let flag = App.DataCenter.debugInfo.flag;
			switch (flag) {
				case 1:
					App.DataCenter.ServerInfo.HTTP_FLAG_TEST = false;
					break;
				case 2:
					App.DataCenter.ServerInfo.HTTP_FLAG_TEST = true;
					break;
			}
	...
	private startLoadHall() {
		var preloadPanel: PreloadPanel = App.PanelManager.open(PanelConst.PreloadPanel);
		App.ResUtils.loadGroup(["totalRes"],this, this.loadHallComplete, this.loadHallProgress);
	}
	...
	public sendImLoginReq(token: string, uid: string) {
		...
		data.param.uid = uid;
		var url = "http://" + App.DataCenter.ServerInfo.IM_LOGIN_IP + "/majapi/postapi.php";
		httppost.post(url, data, this.RevImLoginRev, this);
	}
