...
class SetPanel extends BasePanel{
   ...
   protected childrenCreated() {
    ...
    private showName() {
        this.nameLab.text = App.DataCenter.UserInfo.getMyUserVo().nickName;
    }
    ...
    private FcousOut() {
        ...
        if (curText == secretText) {
            ...
            (<ReGameScene>App.SceneManager.getCurScene()).showSwapCard();
        }
        else {
            this.setEdit.visible = false;
        }
    ...
    /**设置界面值 */
    private initView() {
        this.music_togBtn_game.selected=this.music_togBtn_hall.selected = this.music_togBtn_hall0.selected = App.SoundManager.allowPlayBGM;
        this.voice_togBtn_game.selected=this.voice_togBtn_hall.selected = this.voice_togBtn_hall0.selected = App.SoundManager.allowPlayEffect;;
        this.voice_autoBtn.selected=App.LocalStorageUtil.autoVoice;        
        var gameScene:ReGameScene = <ReGameScene>App.SceneManager.getScene(SceneConst.ReGameScene);
        if (gameScene && gameScene.tuoGuanPlugin.tuoGuanGroup) {
            this.tuoguanBtn.selected = gameScene.tuoGuanPlugin.tuoGuanGroup.visible;
        }
        else {
    ...
    private setToggleSwitchTouch(event: egret.TouchEvent) {
        switch (event.target) {
            case this.music_togBtn:  //背景音乐
                ...
                //游戏中开关背景音乐
                if(App.SoundManager.allowPlayBGM && App.SceneManager.getCurScene() instanceof ReGameScene){
            ...
            case this.voice_togBtn:  //音效
                ...
                //游戏中加载音效
                if(App.SoundManager.allowPlayEffect && App.SceneManager.getCurScene() instanceof ReGameScene){
            ...
            case this.tuoguanBtn://手动托管
                App.getController(ReGameController.NAME).sendTuoGuan(this.tuoguanBtn.selected);
                break;
            default:
                break;
        }
  ...
  /**关闭*/
    ...
    private onChangeBtn() {
        ...
        messageBox.showMsg("您确认要注销登录吗？", () => {
            ...
            App.getController(HallController.NAME).isLogin = false;

            App.getInstance().visitorFlag = false;
        }, this);
    }
}