...
class RulePanelH extends BasePanel{
   ...
   protected childrenCreated() {
    ...
    protected onEnable() {
        ...
        for(var i = 0;i < this.radioA.parent.numChildren;i ++) {
            (<eui.RadioButton>this.radioA.parent.getChildAt(i)).selected = false;
        }
  ...
  /**关闭*/
    ...
    private  setContenText() {
        ...
        this.jp_text_Clearing = <Array<egret.ITextElement>>[
            ...
            ,{text:"\n\n三、扎鸟",style:tetleTextStyleJson}
            ,{text:"\n\n    玩家胡牌后，再摸紧跟着的牌即为鸟。",style:contentTextStyleJson}
            ,{text:"\n\n    设置扎鸟后,必须剩余扎鸟张数量牌用于扎鸟",style:contentTextStyleJson}
            ,{text:"\n\n四、鸟的计算方法",style:tetleTextStyleJson}
            ,{text:"\n\n    以庄家为起点，按牌面数字逆时针计算。譬如，一表示庄家，二表示下家，三为对家，四为上家，依此类推。",style:contentTextStyleJson}
            ,{text:"\n\n    点炮时：",style:contentTextStyleJson}
            ,{text:"\n\n    鸟如果扎在胡牌方或点炮方，即为中鸟，点炮人出双倍分数，扎在其它两个玩家身上，则对积分无影响；",style:contentTextStyleJson}
            ,{text:"\n\n    自摸时：",style:contentTextStyleJson}
            ,{text:"\n\n    鸟如果扎在胡牌方，即所有玩家给双倍分数；",style:contentTextStyleJson}
            ,{text:"\n\n    如果扎在别的玩家，只此一玩家给双倍分数。",style:contentTextStyleJson}
        ];
