        ...
        if (data.code == 200) {
            ...
            if (data.info.reconnect) {
                ...
                if (desk_code != -1) {
                    ...
                    App.DataCenter.deskInfo.deskCode = parseInt(desk_code);
                    ...
                    App.DataCenter.deskInfo.deskCode =1;
 ...
 */
    ...
    private revInRoom(data) {
        ...
        if (!info.retCode) {
            ...
            this.sendSelfRoom()
            TipsLog.hallInfo(ErrorCode.getCodeText(ProtocolHead.Rev102_4_1, info.retCode));
    ...
    public revReconnection(data) {
        ...
        if (json.code == 200) {
            ...
            App.DataCenter.deskInfo.readData(info.deskInfo);   //后端数据不全 会出现undefined
            ...
            // let cur: DeskInfo = App.DataCenter.roomInfo.getCurDesk();
            // console.log("不明所以",cur);
            // App.DataCenter.deskInfo = cur;
            let desk_code = parseInt(info.desk_code);
            //desk_code有时是int 有时是string
            if (desk_code != -1) {
                App.DataCenter.deskInfo.deskCode = parseInt(info.desk_code);
            }else{
                App.DataCenter.deskInfo.deskCode = 1;
            }
    ...
    private revFriendRoomCreate(data) {
        ...
        if (json.code == 200) {
            console.log("创建好友房成功")
            //记录桌子信息
            App.DataCenter.deskInfo.deskCode = json.info.deskCode;
    ...
    private revJoinFriendRoom(data) {
        ...
        if (json.code == 200) {
            ...
            //记录桌子信息
            App.DataCenter.deskInfo.deskCode = data.info.deskCode;
