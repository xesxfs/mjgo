...
class NativeBridge extends SingleClass{
    ...
    private listenLogin(message: string) {
        setTimeout(()=>{
            ...
            if (Number(data.ret)) {
                ...
                if (info.is_visitor+"" == "1") {
                    App.getInstance().visitorFlag = true;
                }
    ...
    private listenBagType(message: string) {
        setTimeout(()=>{
            var data = JSON.parse(message);
            console.log("bagtype&&test===="+message);
            if (data && data.test && Number(data.test) == 2) {
                App.DataCenter.ServerInfo.HTTP_FLAG_TEST = false;
            }
            if (data && data.bag && Number(data.bag) == 1) {
    ...
    private listenIMError(message: string) {
        setTimeout(()=>{
            ...
            if (!data.ret) {
            ...
            else if (data.type == "sendVerificationCode") {
                if(data.ret) {
                    TipsLog.hallInfo("验证码请求已发送");
                }
                else {
            ...
            else if (data.type == "sendVerificationCodeForModify") {
                if(data.ret) {
                    TipsLog.hallInfo("验证码请求已发送");
                }
                else {
                ...
                if(data.ret) {
                    ...
                    if (data.msg) {
                        TipsLog.hallInfo(data.msg);
                    }
                    else {
                        TipsLog.hallInfo("登录错误");
                    }
                }
            }
        }, 100);
     ...
     */
    public sendOffYun() {
        egret.ExternalInterface.call("offYun", "");
    }

    /**
     * 获得、失去输入焦点通知
     */
    public senFocusStatue(data) {
        // var data = {
        //     status: "" //closeKeyboard   openKeyboard
        // }
        egret.ExternalInterface.call("keyboardChange", JSON.stringify(data));
    }
}