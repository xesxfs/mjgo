...
class WXDropDownBOX extends eui.Component{
	...
	public setDataNum(value:string){
		this.dataNum.text = value;
	}
	...
	private selectItem(e:egret.Event){
		...
		if(lab instanceof eui.Label){
		 ...
		 this.changeListType();	
		 if(this.showTxtLab.text=="局数"){
			 NetEventMgr.getInstance().clientMsg(MsgID.Client.WXHHLnningSwitch,"0");
		 }else{
			 NetEventMgr.getInstance().clientMsg(MsgID.Client.WXHHLnningSwitch,"1");
		 }
		}
	}
