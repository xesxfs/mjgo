...
class ReplayLogic{
    ...
    private replayData;              //录像数据
    private tempMyUserInfo:UserVO;   //临时保存自己信息
    private replayTimer:egret.Timer = new egret.Timer(1000);  //录像播放计时器

    private replayControl:ReplayControl; //回放控制
    private replayStatus:ReplayStatus;   //回放状态
    private totalActIndex = 0; //总步数,act数组长度       目前版本没有要求，所以隐藏了
    private curActIndex = 0;   //当前步数，当前重现的act数组索引
     ...
     * @replayData 回放数据
     */
    public play(gameScene:GameScene, revData){
...
//        userInfo.deleteUser(userInfo.httpUserInfo.userID);
        ...
        for(var i=0;i<piLen;i++){
            ...
            var userVo:UserVO = userInfo.getUser(data.userid);
            if(userVo == null){
                userVo = new UserVO();
    ...
    //播放操作
    private playAct(bAnim:boolean = true){
        ...
        if(this.curActIndex > 0){
            ...
            //其他
                ...
                if(actInfo[5] != null){
                    ...
                    //this.gameScene.cancelGang(gangResult);
                }
            } 
        }
        this.replayStatus = ReplayStatus.Free;
    ...
    //回放控制
    private onReplayControl(e:egret.TouchEvent){
        switch (e.target) {
            ...
            case this.replayControl.pauseBtn:
                this.onPause();
                break;
            case this.replayControl.quitBtn:    //结束回放回家
            //将自己的信息重新拿回
                this.resumeMyUserInfo();
    ...
    private shrink(){
        ...
        .call(()=>{
            this.replayControl.openReplayBtn.visible = true;
            this.replayControl.shrinkBtn.visible = false;
        })
    }
    private open() {
        var replayW = this.replayControl.width / 2
        ...
        egret.Tween.get(this.replayControl)
            ...
            .call(() => {
                this.replayControl.openReplayBtn.visible = false;
                this.replayControl.shrinkBtn.visible = true;
