var __reflect = (this && this.__reflect) || function (p, c, t) {
    p.__class__ = c, t ? t.push(c) : t = [c], p.__types__ = p.__types__ ? t.concat(p.__types__) : t;
};
var __extends = (this && this.__extends) || function (d, b) {
    for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p];
    function __() { this.constructor = d; }
    d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
};
/**
 * 吃牌组合
 * @author chenkai
 * @date 2016/7/19
 */
var EatComboUI = (function (_super) {
    __extends(EatComboUI, _super);
    function EatComboUI() {
        var _this = _super.call(this) || this;
        _this.combo3List = []; //组合3的牌组容器
        _this.combo2List = []; //组合2的牌组容器
        _this.cardList = []; //牌数组
        _this.bInitRes = false;
        _this.skinName = "EatComboUISkin";
        return _this;
    }
    EatComboUI.prototype.childrenCreated = function () {
        //初始化鼠标事件
        this.combo3Group.touchEnabled = false;
        this.combo2Group.touchEnabled = false;
        //获取组合容器
        for (var i = 0; i < 2; i++) {
            var group = this.combo2Group.getChildAt(1 + i);
            group.touchChildren = false;
            this.combo2List.push(group);
        }
        for (var i = 0; i < 3; i++) {
            var group = this.combo3Group.getChildAt(1 + i);
            group.touchChildren = false;
            this.combo3List.push(group);
        }
        //居中显示
        this.setCenter();
    };
    EatComboUI.prototype.initRes = function () {
        if (this.bInitRes == false) {
            this.bInitRes = true;
            this.passBtn.setActSkin(ACT_act.Act_Pass);
            this.passBtn0.setActSkin(ACT_act.Act_Pass);
        }
    };
    /**
     * 显示吃牌组合
     * @param comboList 吃牌组合列表
     * @param cardFactory 牌工厂
     * @param invalX 牌间距
     */
    EatComboUI.prototype.showEatCombo = function (comboList, cardFactory, invalX) {
        this.initRes();
        //显示组合UI
        var len = comboList.length;
        if (len == 2) {
            this.combo2Group.visible = true;
            this.combo3Group.visible = false;
        }
        else if (len == 3) {
            this.combo2Group.visible = false;
            this.combo3Group.visible = true;
        }
        else {
            console.error("EatComboUI:吃牌组合数据不正确");
            return;
        }
        console.log(len + "吃牌组合的长度");
        //生成组合牌组
        for (var i = 0; i < len; i++) {
            var cardList = ArrayTool.copyArr(comboList[i]);
            ArrayTool.sortArr(cardList);
            var cardLen = cardList.length;
            for (var j = 0; j < cardLen; j++) {
                var card = cardFactory.getHandCard(cardList[j], UserPosition.Down);
                card.x = -invalX * j;
                card.y = 0;
                if (len == 2) {
                    this.combo2List[i].addChild(card);
                }
                else {
                    this.combo3List[i].addChild(card);
                }
                this.cardList.push(card);
            }
        }
        this.passBtn.playAnim();
        this.addEventListener(egret.TouchEvent.TOUCH_TAP, this.onTouch, this);
    };
    /**
     * 显示杠牌组合
     * @param comboList 杠牌组合列表
     * @param cardFactory 牌工厂
     * @param invalX 牌间距
     */
    EatComboUI.prototype.showGangCombo = function (comboList, cardFactory, invalX) {
        this.initRes();
        //显示组合UI
        var len = comboList.length;
        if (len == 2) {
            this.combo2Group.visible = true;
            this.combo3Group.visible = false;
        }
        else if (len == 3) {
            this.combo2Group.visible = false;
            this.combo3Group.visible = true;
        }
        else {
            console.error("EatComboUI:杠牌组合数据不正确");
            return;
        }
        //生成组合牌组
        for (var i = 0; i < len; i++) {
            var gangValue = comboList[i];
            for (var j = 0; j < 4; j++) {
                var card = cardFactory.getHandCard(gangValue, UserPosition.Down);
                card.x = -invalX * 0.8 * j;
                card.y = 0;
                card.scaleX = 0.8;
                card.scaleY = 0.8;
                if (len == 2) {
                    this.combo2List[i].addChild(card);
                }
                else {
                    this.combo3List[i].addChild(card);
                }
                this.cardList.push(card);
            }
        }
        this.addEventListener(egret.TouchEvent.TOUCH_TAP, this.onTouch, this);
    };
    EatComboUI.prototype.onTouch = function (e) {
        if (e.target instanceof eui.Group) {
            this.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.onTouch, this);
            var group = e.target;
            if (group.parent == this.combo2Group) {
                var index = this.combo2List.indexOf(group);
            }
            else if (group.parent == this.combo3Group) {
                var index = this.combo3List.indexOf(group);
            }
            else {
                console.log("选择吃牌组合时，选择对象不正确");
            }
            this.dispatchEventWith("selectComboEvent", false, index);
            this.hide();
        }
    };
    //清理界面
    EatComboUI.prototype.clear = function () {
        var len = this.cardList.length;
        for (var i = 0; i < len; i++) {
            var card = this.cardList[i];
            card.recycle();
        }
        this.cardList.length = 0;
    };
    EatComboUI.prototype.hide = function () {
        this.parent && this.parent.removeChild(this);
    };
    return EatComboUI;
}(BasePanel));
__reflect(EatComboUI.prototype, "EatComboUI");
//# sourceMappingURL=EatComboUI.js.map